...
const MITRE_TECHNIQUES: Record<string, {
  id: string;
  name: string;
  description: string;
  tactics: string[];
...
// Type definitions
type MITRELookupInput = z.infer<typeof MITRELookupInputSchema>;
type MITRESearchInput = z.infer<typeof MITRESearchInputSchema>;
type ExerciseCreateInput = z.infer<typeof ExerciseCreateInputSchema>;
type CoverageMapInput = z.infer<typeof CoverageMapInputSchema>;

// In-memory exercise storage (would use database in production)
const exercises: Map<string, {
  id: string;
  name: string;
  description: string;
  techniques: string[];
  ...
  created: string;
  findings: Array<{ technique: string; detected: boolean; notes: string }>;
}> = new Map();

export function registerPurpleTeamTools(server: McpServer): void {
    ...
    async (params: MITRESearchInput) => {
      try {
        ...
        const matches = Object.values(MITRE_TECHNIQUES).filter(t => {
          ...
          if (params.tactic) {
            return matchesKeyword && t.tactics.some(t => 
              t.toLowerCase().includes(params.tactic!.toLowerCase())
            );
          }
...
Note:
  ...
  - Use techniques from cyber_mitre_search`,
    ...
    async (params: ExerciseCreateInput) => {
      try {
  ...
  - Full ATT&CK matrix has 200+ techniques`,
    ...
    async (params: CoverageMapInput) => {
      try {
        ...
        // Group by tactic
        const tacticCoverage: Record<string, { total: number; detected: number }> = {};
