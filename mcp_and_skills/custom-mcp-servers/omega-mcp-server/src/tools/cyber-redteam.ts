...
// Type definitions
type SubdomainEnumInput = z.infer<typeof SubdomainEnumInputSchema>;
type PortScanInput = z.infer<typeof PortScanInputSchema>;
type DNSAnalyzeInput = z.infer<typeof DNSAnalyzeInputSchema>;
type WHOISInput = z.infer<typeof WHOISInputSchema>;
type HeadersAnalyzeInput = z.infer<typeof HeadersAnalyzeInputSchema>;

// Helper functions
async function enumerateSubdomainsCT(domain: string): Promise<Array<{ subdomain: string; source: string }>> {
...
async function resolveDNS(domain: string): Promise<Array<{ type: string; value: string }>> {
...
async function performWHOIS(domain: string): Promise<string> {
  try {
...
async function analyzeHeaders(url: string): Promise<{
  headers: Record<string, string>;
  securityHeaders: Record<string, { present: boolean; value?: string }>;
  ...
  const securityHeaders: Record<string, { present: boolean; value?: string }> = {};
  let score = 0;
...
export function registerRedTeamTools(server: McpServer): void {

  // Subdomain Enumeration
  server.registerTool(
    "cyber_subdomain_enum",
    {
...
Returns:
  Security header analysis with score and recommendations.`,
    ...
    async (params: HeadersAnalyzeInput) => {
      try {
