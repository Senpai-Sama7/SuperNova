...
// Type definitions
type ThreatIPLookupInput = z.infer<typeof ThreatIPLookupInputSchema>;
type ThreatDomainLookupInput = z.infer<typeof ThreatDomainLookupInputSchema>;
type ThreatHashLookupInput = z.infer<typeof ThreatHashLookupInputSchema>;
type CVELookupInput = z.infer<typeof CVELookupInputSchema>;
type CVESearchInput = z.infer<typeof CVESearchInputSchema>;
...
async function getVirusTotalIP(ip: string, apiKey?: string): Promise<{
  malicious: number;
  suspicious: number;
  harmless?: number;
  undetected?: number;
...
async function getCVEInfo(cveId: string): Promise<{
  id: string;
  description: string;
  severity: string;
  cvssScore: number;
  ...
  try {
    ...
    return {
      id: cve.id,
      description: cve.descriptions?.find((d: { lang: string }) => d.lang === 'en')?.value || 'No description',
...
async function searchCVEs(keyword: string, limit: number): Promise<Array<{
  id: string;
  description: string;
  severity: string;
  published: string;
...
export function registerBlueTeamTools(server: McpServer): void {

  // IP Threat Lookup
  server.registerTool(
    "cyber_threat_ip_lookup",
    {
...
Note:
  ...
  - Rate limits apply to free tiers`,
    ...
    async (params: ThreatIPLookupInput) => {
      try {
        ...
        let riskScore = 0;
        const vt = results.virustotal as { malicious?: number; suspicious?: number; harmless?: number; undetected?: number } | null;
...
## Geolocation
${results.ipinfo ? `
- **City**: ${(results.ipinfo as {city?: string}).city || 'N/A'}
- **Region**: ${(results.ipinfo as {region?: string}).region || 'N/A'}
- **Country**: ${(results.ipinfo as {country?: string}).country || 'N/A'}
- **Organization**: ${(results.ipinfo as {org?: string}).org || 'N/A'}
` : 'N/A'}

## VirusTotal${!results.virustotal || (results.virustotal as {error?: string}).error ? ' (API key required)' : ''}
${vt && !(vt as {error?: string}).error ? `
- **Malicious**: ${vt.malicious || 0} detections
- **Suspicious**: ${vt.suspicious || 0}
- **Harmless**: ${vt.harmless || 0}
- **Undetected**: ${vt.undetected || 0}
` : (results.virustotal as {error?: string})?.error || 'Not queried'}`;
...
Returns:
  Domain reputation analysis and threat indicators.`,
    ...
    async (params: ThreatDomainLookupInput) => {
      try {
        // Get DNS records
        ...
        try {
          ...
          indicators.has_spf = txt.some(records => 
...
Example:
  - hash="e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"`,
    ...
    async (params: ThreatHashLookupInput) => {
      try {
        // Detect hash type
        let hashType = 'unknown';
        if (params.hash.length === 32) hashType = 'MD5';
        else if (params.hash.length === 40) hashType = 'SHA1';
        else if (params.hash.length === 64) hashType = 'SHA256';
