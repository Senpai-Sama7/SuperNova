...
Error Handling:
  ...
  - Requires GITHUB_TOKEN environment variable for higher rate limits`,
        ...
        try {
            const result = await client.searchRepos(params.query, params.sort, params.order, params.limit, 1);
...
Error Handling:
  ...
  - Returns empty list if no matching issues`,
        ...
        try {
            const issues = await client.listIssues(params.owner, params.repo, params.state, params.labels, params.assignee, params.limit, params.page);
            const formatMarkdown = (data) => {
...
Error Handling:
  ...
  - Requires GITHUB_TOKEN with repo access`,
        ...
        try {
            const issue = await client.createIssue(params.owner, params.repo, params.title, params.body, params.labels, params.assignees);
            const formatMarkdown = (data) => {
  ...
  - Requires GITHUB_TOKEN with repo access`,
        ...
        try {
            ...
            if (params.assignees !== undefined)
                updates.assignees = params.assignees;
            const issue = await client.updateIssue(params.owner, params.repo, params.issue_number, updates);
            const formatMarkdown = (data) => {
                const labels = data.labels.length ? ` [${data.labels.join(", ")}]` : "";
                return [
                    `# ✅ Issue Updated`,
...
Error Handling:
  - Returns error if repository not found`,
        ...
        try {
            const prs = await client.listPullRequests(params.owner, params.repo, params.state, params.head, params.base, params.limit, params.page);
            const formatMarkdown = (data) => {
...
Error Handling:
  - Returns error if PR not found`,
    ...
    // Create PR review
    server.registerTool("github_create_pull_request_review", {
...
Error Handling:
  ...
  - Requires GITHUB_TOKEN with repo access`,
        ...
        try {
            const review = await client.createPullRequestReview(params.owner, params.repo, params.pull_number, params.event, params.body);
            const formatMarkdown = (data) => {
                const actionLabels = {
                    APPROVE: "✅ Approved",
