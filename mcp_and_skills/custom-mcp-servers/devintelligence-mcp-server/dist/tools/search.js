  ...
  - Returns error if base path not found`,
        ...
        try {
            ...
            const formatMarkdown = (data) => {
                ...
                if (params.include)
                    lines.push(`**Include**: ${params.include}`);
                if (params.exclude)
                    lines.push(`**Exclude**: ${params.exclude}`);
    ...
    server.registerTool("search_find_definitions", {
  ...
  - May return false positives for common names`,
        ...
        try {
            ...
            switch (params.language) {
                case "typescript":
                case "javascript":
                    patterns.push(`function\\s+${params.symbol}\\b`, `(const|let|var)\\s+${params.symbol}\\s*[=:]`, `class\\s+${params.symbol}\\b`, `interface\\s+${params.symbol}\\b`, `type\\s+${params.symbol}\\b`, `${params.symbol}\\s*[=:]\\s*(async\\s*)?\\(`, `async\\s+function\\s+${params.symbol}\\b`);
                    extensions.push("ts", "tsx", "js", "jsx", "mjs");
                    break;
                case "python":
                    patterns.push(`def\\s+${params.symbol}\\b`, `class\\s+${params.symbol}\\b`, `${params.symbol}\\s*=`);
                    extensions.push("py", "pyi");
                    break;
                case "go":
                    patterns.push(`func\\s+\\(?.+\\)?\\s*${params.symbol}\\b`, `func\\s+${params.symbol}\\b`, `type\\s+${params.symbol}\\b`, `var\\s+${params.symbol}\\b`, `const\\s+${params.symbol}\\b`);
                    extensions.push("go");
                    break;
                case "rust":
                    patterns.push(`fn\\s+${params.symbol}\\b`, `struct\\s+${params.symbol}\\b`, `impl\\s+.*\\b${params.symbol}\\b`, `trait\\s+${params.symbol}\\b`, `const\\s+${params.symbol}\\b`, `let\\s+${params.symbol}\\b`);
                    extensions.push("rs");
                    break;
                case "java":
                    patterns.push(`(public|private|protected)?\\s*(static)?\\s*(final)?\\s*\\w+\\s+${params.symbol}\\b`, `class\\s+${params.symbol}\\b`, `interface\\s+${params.symbol}\\b`, `enum\\s+${params.symbol}\\b`);
                    extensions.push("java");
                    break;
                default:
                    // Generic patterns for any language
                    patterns.push(`function\\s+${params.symbol}\\b`, `def\\s+${params.symbol}\\b`, `class\\s+${params.symbol}\\b`, `fn\\s+${params.symbol}\\b`, `(const|let|var)\\s+${params.symbol}\\b`, `${params.symbol}\\s*=`);
            ...
            for (const line of lines) {
                try {
                    ...
                    if (data.type === "match") {
                        ...
                        let type = "unknown";
                        if (code.match(/\bfunction\b|\bdef\b|\bfn\b/))
                            type = "function";
                        else if (code.match(/\bclass\b/))
                            type = "class";
                        else if (code.match(/\binterface\b/))
                            type = "interface";
                        else if (code.match(/\b(const|let|var)\s+\w+\s*[:=]/))
                            type = "variable";
                        else if (code.match(/\b\w+\s*=/))
                            type = "assignment";
                        definitions.push({
                            file: filePath,
                            line: lineNum,
                        ...
                        if (definitions.length >= params.limit)
                            break;
                    }
                }
                catch {
